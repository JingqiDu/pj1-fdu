...
class Model_CNN(Layer):
    ...
    def __init__(self):
        ...
        self.layers = [
            conv2D(1, 8, 3, stride=2, padding=1, initialize_method=init),
            ReLU(),
            conv2D(8, 16, 3, stride=2, padding=1, initialize_method=init),
            ReLU(),
            Linear(16 * 7 * 7, 10, initialize_method=init)
        ]
    ...
    def backward(self, loss_grad):
        grads = self.layers[4].backward(loss_grad)
    ...
    def load_model(self, param_list):
        ...
        for layer in self.layers:
            if layer.optimizable:
                ...
                layer.params['W'] = layer.W
                layer.params['b'] = layer.b
                layer.weight_decay = params[param_idx].get('weight_decay', False)
                layer.weight_decay_lambda = params[param_idx].get('lambda', 1e-8)
                param_idx += 1
    ...
    def save_model(self, save_path):
        ...
        for layer in self.layers:
            if layer.optimizable:
                param_list.append({'W' : layer.params['W'], 'b' : layer.params['b'], 'weight_decay' : layer.weight_decay, 'lambda' : layer.weight_decay_lambda})
        with open(save_path, 'wb') as f:
            pickle.dump({'model': 'cnn', 'params': param_list}, f)
